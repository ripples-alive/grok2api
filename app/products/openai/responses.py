...
async def create(
    *,
    ...
    if tools:
        ...
        logger.info(
            "responses tool injection: tool_names={} choice={}", tool_names, tool_choice
        )
                            ...
                            output.append(
                                {
                                    ...
                                    "type": "reasoning",
                                    "summary": [
                                        {"type": "summary_text", "text": full_think}
                                    ],
                                    "status": "completed",
                                }
                            )
    ...
    if tool_names:
        ...
        if tc_result.calls:
            ...
            rt = estimate_tokens(full_think) if full_think else 0
            logger.info(
                "responses tool_calls: model={} calls={}", model, len(tc_result.calls)
            )
    ...
    logger.info(
        "responses request completed: model={} text_len={} reasoning_len={} image_count={}",
        model,
        len(full_text),
        len(full_think),
        len(adapter.image_urls),
